...
, inverse
  ...
  where
    ...
    complement aps = fmap (Tuple.uncurry APoint.mk) . APerm.Inner.Annotation.inject m $ complementAux ys
      where
    ...
    reversalComplement aps = fmap (Tuple.uncurry APoint.mk) . APerm.Inner.Annotation.inject m $ reversalComplementAux ys
      where
        ps = fmap APoint.point aps
        ys = fmap Point.yCoord ps
        as = fmap APoint.annotation aps
    ...
    inverse aps = fmap (Tuple.uncurry APoint.mk) . APerm.Inner.Annotation.inject m $ inverseAux ys
      where
        ps = fmap APoint.point aps
        ys = fmap Point.yCoord ps
        as = fmap APoint.annotation aps
